...
export const Header = (props) => {
  ...
  return (
    <header
      className={className}
    >
      <div className={"header__logo"}><img src={logo} alt="background-tablet"></img></div>
        <div className={'header__navigation'}>
         <NavbarContext.Provider value={{ navbarOpen, setNavbarOpen }}>
            ...
            <Menu open={navbarOpen} openFunc={setNavbarOpen} className={`${isActive ? 'none' : ''}`}></Menu>
            <FontAwesomeIcon icon={faXmark}  className={`${open ? 'open' : ``} `}  onClick={handleClick} />
